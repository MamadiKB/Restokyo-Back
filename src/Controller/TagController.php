...
class TagController extends AbstractController
{
    ...
    {   
        // Crée un nouvel objet Tag vide
        $tag = new tag ;
        // Crée un formulaire de type TagType à partir de l'objet Tag vide
        ...
        $form->submit($data);
        // Si le formulaire est soumis et valide, déserialise les données JSON de la requête en un objet Tag
        if ($form->isSubmitted() && $form->isValid()) {
            // Déserialise les données JSON de la requête en un objet Tag
            $newTag = $serializer->deserialize($request->getContent(),
            Tag::class,
            'json',
            [AbstractNormalizer::OBJECT_TO_POPULATE => $tag]);
            // Enregistre l'objet Tag dans la base de données
            $em->persist($newTag);
            $em->flush();

            $jsonTag = $serializer->serialize($newTag, 'json', ['groups' => 'getTag']);
        ...
        // Si le formulaire n'est pas valide, renvoie les erreurs au client sous forme de JSON
                              ...
                              ValidatorInterface $validator): JsonResponse 
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $jsonTag = $serializer->serialize($updatedTag, 'json', ['groups' => 'getTag']);
